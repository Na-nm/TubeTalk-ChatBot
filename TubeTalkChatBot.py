# chainlit run TubeTalkChatBot.py 
...
def create_new_session():
    ...
    print("New session..." , session_dir)
    os.makedirs(session_dir, exist_ok=True)

def clean_session():
    global session_dir, vectorstore, full_transcript
    print(f"Session info  ...... {session_dir}  and  {vectorstore}")
    if vectorstore:
        try:
            vectorstore._collection = None
            vectorstore = None
            print("Vectore store >>>>>>> ", vectorstore)
        except:
            pass
    if session_dir and os.path.exists(session_dir):
        print(f"Session directory {session_dir} cleaned up.") 
        # Delete the session directory and its contents from the CPU
        #shutil.rmtree(session_dir) 
    session_dir = None
    print("Session deleted succesfully >>>>>>>>>> ", session_dir)
    full_transcript = ""
...
def build_agent(vectorstore, transcript):
    ...
    tools = [
        ...
        description="Use this tool to search for answers from the video transcript,\
            if the question is not included in the video transcript response with:\
            This question isn't included in the video, ask another question please!."
            )
    ]
...
async def handle_message(message: cl.Message):
    ...
    if state == "awaiting_link":
        ...
        try:
            ...
            if not re.match(youtube_pattern, message.content.strip()):
